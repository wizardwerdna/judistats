...
require File.expand_path(File.dirname(__FILE__) + '/../../lib/checker/PSFile')

describe PSFile, "whe opened on an empty file" do
  it "should complain" do
    lambda{@psfile = PSFile.open(File.dirname(__FILE__) + '/file_empty.txt')}.should raise_error
  end
end

describe PSFile, "when opened on a single-hand file" do
  ...
  before do
    @psfile = PSFile.open(File.dirname(__FILE__) + '/file_one_hand.txt')
  end
...
describe PSFile, "when opened on a multi-hand file" do
  ...
  before do
    @psfile = PSFile.open(File.dirname(__FILE__) + '/file_many_hands.txt')
  ...
  it "should collect entries with all the proper information" do
    ...
    @psfile.entries.each do |handrecord|
      handrecord.path.should == @expanded_path
      handrecord.starting_at.should == list.shift
    end
  end
