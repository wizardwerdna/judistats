...
describe HandStatistics, "when reporting statistics" do
  ...
  it "should report blind attack statistics for each player" do
    @blind_attack_statistics_holder.should_receive(:blind_attack_opportunity?).exactly(@stats.players.size)
    @blind_attack_statistics_holder.should_receive(:blind_attack_opportunity_taken?).exactly(@stats.players.size)
    @blind_attack_statistics_holder.should_receive(:blind_defense_opportunity?).exactly(@stats.players.size)
    @blind_attack_statistics_holder.should_receive(:blind_defense_opportunity_taken?).exactly(@stats.players.size)
    @reports = @stats.reports
    @stats.players.each{|each| @reports[each].should include(:is_blind_attack_opportunity)}
    @stats.players.each{|each| @reports[each].should include(:is_blind_attack_opportunity_taken)}
    @stats.players.each{|each| @reports[each].should include(:is_blind_defense_opportunity)}
    @stats.players.each{|each| @reports[each].should include(:is_blind_defense_opportunity_taken)}
  end
  
  it "should report continuation bet statistics for each player" do
    @continuation_bet_statistics_holder.should_receive(:cbet_opportunity?).exactly(@stats.players.size)
    @continuation_bet_statistics_holder.should_receive(:cbet_opportunity_taken?).exactly(@stats.players.size)
    @reports = @stats.reports
  ...
  it "should report cash statistics for each player" do
    @cash_statistics_holder.should_receive(:posted).exactly(@stats.players.size)
    @cash_statistics_holder.should_receive(:paid).exactly(@stats.players.size)
    @cash_statistics_holder.should_receive(:won).exactly(@stats.players.size)
    @cash_statistics_holder.should_receive(:cards).exactly(@stats.players.size)
    @reports = @stats.reports
  ...
  it "should report aggression statistics for each player" do
    @aggression_statistics_holder.should_receive(:preflop_passive).exactly(@stats.players.size)
    @aggression_statistics_holder.should_receive(:preflop_aggressive).exactly(@stats.players.size)
    @aggression_statistics_holder.should_receive(:postflop_passive).exactly(@stats.players.size)
    @aggression_statistics_holder.should_receive(:postflop_aggressive).exactly(@stats.players.size)
    @reports = @stats.reports
  ...
  it "should report preflop raise statistics for each player" do
    @preflop_raise_statistics_holder.should_receive(:pfr_opportunity?).exactly(@stats.players.size)
    @preflop_raise_statistics_holder.should_receive(:pfr_opportunity_taken?).exactly(@stats.players.size)
    @reports = @stats.reports
...
describe HandStatistics, "when registering game activity" do
  ...
  it "should allow you to register a player" do
    ...
    @stats.should have(1).player_hashes
