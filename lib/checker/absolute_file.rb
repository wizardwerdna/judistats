...
class AbsoluteFile
  ...
  ABSOLUTE_HEADER_PATTERN = /Stage #([0-9]+)/
  ...
  def pos=(index)
    ...
    unless @lastline && @lastline =~ ABSOLUTE_HEADER_PATTERN
      close
      raise "hand record must begin with a valid header line for absolute"
    end
    @lines = [@lastline]
  end
  ...
  def next_handrecord
    ...
    until @file.eof?
      @lastline = read_and_chomp_next_line_from_file
      break if @lastline =~ ABSOLUTE_HEADER_PATTERN
      @lines << @lastline unless @lastline.empty?
    end
    result, @lines = HandHistory.new(@lines, @filename, starting_at, AbsoluteHandHistoryParser), [@lastline]
    if @file.eof?
      @lastline = nil
